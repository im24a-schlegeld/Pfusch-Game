...
export function trafficWaveSpacing(initial: number, minimum: number, difficulty: number) {
...
export function trafficArrivalGap(
  riderSpeed: number, previousSpeed: number, nextSpeed: number,
  previousHalfLength: number, nextHalfLength: number,
) {
  const previousClosing = Math.max(5, riderSpeed - previousSpeed);
  const nextClosing = Math.max(5, riderSpeed - nextSpeed);
  return Math.max(1.5,
    TRAFFIC_FLOW.reactionSeconds + TRAFFIC_FLOW.laneChangeSeconds
      + previousHalfLength / previousClosing + nextHalfLength / nextClosing,
  );
}
