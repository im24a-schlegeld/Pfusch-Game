...
test('production assets, console, narrow screens and saved customization', async ({ page, request }) => {
  const errors: string[] = [];
  ...
  for (const size of [{width:360,height:640}, {width:390,height:844}, {width:1280,height:720}]) {
    await page.setViewportSize(size);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    await expect(page.getByRole('button', {name:'LET’S RIDE',exact:true})).toBeVisible();
  }
  await page.getByRole('navigation').getByRole('button',{name:'REWARDS',exact:true}).click();
  ...
  await page.getByRole('navigation').getByRole('button',{name:'GARAGE',exact:true}).click();
  await page.getByRole('tab',{name:'BIKE',exact:true}).click();
  await page.getByRole('button',{name:'Asphalt paint',exact:true}).click();
  await expect(page.getByRole('button',{name:'Asphalt paint',exact:true})).toHaveAttribute('aria-pressed','true');
  await page.reload();
