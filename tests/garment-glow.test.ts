...
describe('Keep Up ink emission', () => {
  ...
  it('allows only the three stable Keep Up IDs and starts with no daylight emission', () => {
    ...
    for (const id of [undefined, 'keep-up-hoodie', '10237061759305-copy', 'racing-zipper']) {
      const material = new THREE.MeshStandardMaterial();
  ...
  it('caches shared garment materials once and returns exactly to zero in daylight', () => {
    ...
    const ordinary = new THREE.MeshStandardMaterial({ emissiveIntensity: 0.07 });
    ...
    expect(material.emissiveIntensity).toBeLessThan(0.22);
    update(1);
